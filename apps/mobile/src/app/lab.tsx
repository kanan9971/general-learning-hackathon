...
export default function MarketLabScreen() {
  ...
  const { section, kinds } = useLocalSearchParams<{ section?: MarketSectionId; kinds?: string }>();
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const fb = await answerMarketLab({ question_id: q.id, answer, level, section, watch });
      setFeedback(fb);
      setResults((r) => [...r, { q, fb }]);
    } catch (e) {
  ...
  if (!q) return <Summary results={results} onAgain={() => void load()} onDone={() => router.back()} router={router} />;

  const total = set.questions.length;
  return (
    <Screen
      title={`Question ${i + 1} of ${total}`}
      subtitle="Today's market, tested"
      safeEdges={['bottom']}
      footer={
        <>
          {feedback ? (
            <PrimaryButton label={i + 1 < total ? 'Next question' : 'See my results'} onPress={next} />
          ) : (
            <PrimaryButton
              label={
    ...
    >
      <View style={styles.topRow}>
        <DataModeBadge mode={set.data_mode} asOf={asOfLabel(set.as_of)} />
        <View style={styles.dots}>
          {set.questions.map((_, n) => (
            <View
              key={n}
...
function Summary({
  ...
  return (
    <Screen
      ...
      footer={
        <>
          <PrimaryButton label="Another set" onPress={onAgain} />
          <PrimaryButton label="Back to Markets" variant="secondary" onPress={onDone} />
          <Disclaimer />
        </>
      }
    >
